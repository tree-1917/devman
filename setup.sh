...
readonly REMOTE_BASE_URL="https://raw.githubusercontent.com/tree-1917/Snama/main"
...
# ------------------------------------------------------------------------------
detect_pkg_manager() {
    # On macOS, prefer Homebrew; on Linux, prefer native package managers
...
# ------------------------------------------------------------------------------
# Clone / update plugins and themes
# ------------------------------------------------------------------------------
...
clone_or_update() {
    ...
    if [ -d "$dest_dir/.git" ]; then
        ...
        if git -C "$dest_dir" pull --ff-only 2>/dev/null; then
            ...
            log_warn "$name pull failed (local changes?). Skipping update."
        fi
    else
...
# ------------------------------------------------------------------------------
setup_zshrc() {
    local local_file="$SCRIPT_DIR/.zshrc"
    local remote_url="$REMOTE_BASE_URL/zshrc"
    ...
    if [ -f "$local_file" ]; then
        ...
        log_ok "Copied local .zshrc configuration."
    else
        log_warn "No local .zshrc found in $SCRIPT_DIR. Downloading from remote..."
        if curl -fsSL "$remote_url" -o "$target"; then
            log_ok "Remote .zshrc applied successfully."
        else
            log_error "Failed to download .zshrc from remote repo."
            exit 1
        fi
    fi
}
...
# ------------------------------------------------------------------------------
setup_snippets() {
    local local_file="$SCRIPT_DIR/.zsh_snippets_history"
    local remote_url="$REMOTE_BASE_URL/zsh_snippets_history"
    local target="$HOME/.zsh_snippets_history"

    if [ -f "$local_file" ]; then
        cp "$local_file" "$target"
        log_ok "Copied provided .zsh_snippets_history configuration."
    elif [ -f "$target" ]; then
        ...
        if curl -fsSL "$remote_url" -o "$target"; then
            log_ok "Remote .zsh_snippets_history downloaded successfully."
        else
...
set_default_shell() {
    ...
    # Ensure zsh is in /etc/shells (required by chsh on macOS and many Linux distros)
    if ! grep -qx "$zsh_path" /etc/shells 2>/dev/null; then
        log_warn "$zsh_path not found in /etc/shells. Adding it..."
        if cmd_exists sudo; then
            echo "$zsh_path" | sudo tee -a /etc/shells >/dev/null
        else
            log_warn "sudo not available. You may need to run manually:"
            log_warn "  echo $zsh_path | sudo tee -a /etc/shells"
        fi
    fi

    log_info "Setting zsh as default shell..."
    if chsh -s "$zsh_path" 2>/dev/null; then
        log_ok "Zsh is now the default shell."
